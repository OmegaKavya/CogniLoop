...
\begin{itemize}
    ...
    \item \textbf{Phase 2 (Upcoming Pilot)}: Controlled IRB-approved human participant classroom trials ($N \ge 20$) evaluating real pre/post learning gains, cognitive load, and delayed retention.
    \item \textbf{Phase 3 (Longitudinal)}: Multi-month enterprise deployment evaluating long-term retention and cluster migration.
\end{itemize}
...
\end{itemize}
The assigned cluster $C \in \{C_0, C_1, C_2\}$ forms a core dimension of the contextual bandit state. State discretization is an intentional engineering trade-off designed to mitigate cold-start sparsity and maintain sufficient observations per context-action pair.
...
\begin{enumerate}
    ...
    \item \textbf{Semantic Grounding Check}: Confirms that the correct answer and distractor tokens overlap with the retrieved transcript context window.
    \item \textbf{Distractor Uniqueness}: Rejects items with duplicate options or ambiguous keys. If validation fails, execution deterministically falls back to Tier 3.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Normalized Reward Formulation}: Normalized score reward reflects raw performance $r = \frac{\text{Score}}{100.0} \in [0, 1]$. To prevent double-counting difficulty, difficulty utility weights $w_a \in \{w_{\text{easy}}=0.8, w_{\text{med}}=1.0, w_{\text{hard}}=1.2\}$ are applied exclusively during action selection.
...
\subsection{Phase 1 Simulation Protocol}
To evaluate algorithmic convergence prior to human classroom deployment, we executed a simulation across $N=60$ student profiles (random seed $42$). Profiles were partitioned into three equal archetypes ($N=20$ each):
\begin{itemize}
    \item \textbf{Fast Archetype}: High initial baseline ($P(L_0) \in [0.45, 0.58]$), low slip rate.
    \item \textbf{Standard Archetype}: Moderate baseline ($P(L_0) \in [0.30, 0.45]$), baseline pacing.
    \item \textbf{Slow Archetype}: Low baseline ($P(L_0) \in [0.15, 0.30]$), higher slip rate.
\end{itemize}

Profiles were randomized equally into an \textbf{Experimental Group} ($N_e=30$, CogniLoop adaptive RAG + DC-BKT + Thompson Sampling) and a \textbf{Control Group} ($N_c=30$, static sequential video viewing without difficulty adaptation). At each step, response outcomes were generated via an independent observation model:
\begin{equation}
P(\text{Correct}) = P(L_n)(1 - P(S_a)) + (1 - P(L_n))P(G_a)
\end{equation}
...
To address whether simulation findings are sensitive to heuristic parameter selection, we perturbed $P(G)$, $P(S)$, and $P(T)$ by $\pm 0.05$ across 100 simulation runs. Across all configurations:
\begin{itemize}
    \item Mean experimental NLG remained within $[0.742, 0.815]$.
    \item Mean control NLG remained within $[0.210, 0.268]$.
    \item In all perturbed regimes, Welch's $t$-test remained significant ($p < 0.001$), demonstrating that CogniLoop's adaptive advantage is robust to parameter perturbations.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Ablation B (Zero-Shot LLM - No RAG Grounding)}: Increased context mismatch from $2.1\%$ to $28.4\%$.
...
\begin{enumerate}
    \item \textbf{Csikszentmihalyi's Flow Channel}: By sampling from updated Beta distributions, Thompson Sampling dynamically balances challenge and skill, preventing boredom (easy items given to fast learners) and anxiety (hard items given to struggling learners).
