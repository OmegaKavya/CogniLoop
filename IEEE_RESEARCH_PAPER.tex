...
\begin{itemize}
    ...
    \item \textbf{Contextual Multi-Armed Bandit Policy}: We formulate difficulty routing as a contextual bandit problem optimized via Thompson Sampling using Beta-distributed posterior sampling with continuous fractional reward updates, mapping multidimensional learner states directly to optimal challenge levels.
    \item \textbf{Simulation Rigor, Sensitivity, and Ablation}: We evaluate the architecture under a fully formalized simulation response model ($N=60$) across three learner archetypes, reporting Welch's $t$-test, Mann-Whitney $U$, and effect size metrics (Cohen's $d = 0.973$), accompanied by a multi-parameter sensitivity analysis and a six-configuration ablation study.
\end{itemize}
...
\end{enumerate}
\textbf{Validation Failure Control Flow}: If generation fails validation, the system executes a single fast regeneration retry. If the retry fails validation again, execution deterministically falls back to Tier 3 (the pre-validated domain pool), preventing any broken or hallucinatory UI states.
...
\begin{enumerate}
    ...
    \item \textbf{Normalized Reward Formulation}: Normalized score reward reflects raw performance $r = \frac{\text{Score}}{100.0} \in [0, 1]$. To prevent double-counting difficulty, difficulty utility weights $w_a \in \{w_{\text{easy}}=0.8, w_{\text{med}}=1.0, w_{\text{hard}}=1.2\}$ are applied exclusively during action selection. These utility weights are pedagogically grounded in Vygotsky's Zone of Proximal Development (ZPD) and Csikszentmihalyi's flow channel—placing a higher instructional premium on mastering challenging conceptual items over trivial recall.

    \item \textbf{Beta Posterior Updates via Fractional Observations}: For each state-action pair $(s, a)$, the system maintains success parameters $\alpha_{s,a}$ and failure parameters $\beta_{s,a}$, initialized with uninformative priors $\alpha_0 = 1.0, \beta_0 = 1.0$. Upon observing continuous reward $r \in [0, 1]$:
    \begin{equation}
    ...
    \item \textbf{Action Selection via Posterior Sampling}: At each checkpoint, for each candidate action $a \in \mathcal{A}$, an expected performance sample $\theta_a$ is drawn from its posterior Beta distribution:
    \begin{equation}
...
\caption{Simulated Normalized Learning Gain (NLG) across learner archetypes comparing CogniLoop adaptive tutoring against static video instruction ($N=60$, Welch's $t = 3.768, p < 0.001, d = 0.973$).}
\label{fig_results}
\end{figure}
...
Crucially, the simulated learner response generator operates as an independent environment stochastic process separated from the DC-BKT Bayesian estimator and bandit policy in CogniLoop. At each step $n$, a learner's actual latent understanding $L_n$ evolves, and their observable binary response $Y_n \in \{0, 1\}$ is generated according to:
\begin{equation}
...
\midrule
\textbf{Welch's $t$-test (Unequal Var)} & \multicolumn{3}{c}{$t = 3.768, \quad p = 4.73 \times 10^{-4} < 0.001$} \\
\textbf{Mann-Whitney $U$} & \multicolumn{3}{c}{$U = 709.5, \quad p = 9.50 \times 10^{-5} < 0.0001$} \\
\textbf{Cohen's $d$ Effect Size} & \multicolumn{3}{c}{$d = 0.973 \quad (\text{Large Effect Magnitude}, d > 0.8)$} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\subsection{Multi-Parameter Sensitivity Analysis}
To address whether simulation findings depend on specific heuristic parameters, we perturbed $P(G)$, $P(S)$, and $P(T)$ by $\pm 0.05$ across 100 simulation runs. Across the evaluated perturbation regimes:
\begin{itemize}
    \item The experimental condition consistently produced higher simulated NLG than the control condition (Exp NLG remained in $[0.742, 0.815]$ vs. Control NLG in $[0.210, 0.268]$).
    \item In all perturbed regimes, Welch's $t$-test remained significant ($p < 0.001$).
    \item Comparing weighted difficulty utilities $(0.8, 1.0, 1.2)$ against uniform utility $(1.0, 1.0, 1.0)$ showed that utility weighting accelerated mastery convergence for fast learners by $18.4\%$ without increasing slip rates.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Bandit Posterior Sampling}: $0.18\text{ ms} \pm 0.02\text{ ms}$.
