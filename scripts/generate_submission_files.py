...
def build_native_ieee_pdf():
    print("Compiling native IEEE double-column PDF with embedded figures via tectonic...")
    cmd = "./tectonic IEEE_RESEARCH_PAPER.tex --outdir Submission"
    res = os.system(cmd)
    if res == 0:
        print("Successfully generated: Submission/IEEE_RESEARCH_PAPER.pdf (Native IEEE Double-Column)")
    else:
        print("Warning: Tectonic compilation failed. Preserving existing PDF.")

def main():
    print("Generating official submission package in 'Submission/' directory...")

    # Step 1: Generate high-resolution diagram and result figures
    os.system("python3 scripts/generate_diagram_figures.py")

    # Step 2: Build native IEEE double-column PDF via tectonic
    build_native_ieee_pdf()

    # Step 3: Build auxiliary PDFs (Architecture, Simulation Report, README)
    ...
    # Step 4: Master Combined PDF Package (CogniLoop_v1.0.pdf)
    master_sources = [
    ...
    # Step 5: PowerPoint Presentation (Presentation.pptx)
    ...
    for fname in os.listdir("Submission"):
        ...
        print(f" - Submission/{fname} ({size_kb:.1f} KB)")
